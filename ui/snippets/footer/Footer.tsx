...
const FRONT_VERSION_URL = `https://github.com/Gemefi/frontend/tree/${ config.UI.footer.frontendVersion }`;
const FRONT_COMMIT_URL = `https://github.com/Gemefi/frontend/commit/${ config.UI.footer.frontendCommit }`;
  ...
  const renderProjectInfo = React.useCallback((gridArea?: GridProps['gridArea']) => {
    return (
      <Box gridArea={ gridArea }>
        <Link fontSize="xs" href="https://gemefi.io">gemefi.io</Link>
        <Text mt={ 3 } fontSize="xs">
          Gemefi is an EVM compatible Layer 1 to build for the future of Web3 ecosystem in the cryptocurrency realm, security and mass-adoption with Web3 industry.
        </Text>
    ...
    return (
      <Grid { ...containerProps }>
        ...
        >
          {
            ([
              { title: 'Gemefi', links: Gemefi_LINKS },
              ...(linksData || []),
            ])
              .slice(0, colNum)
              .map(linkGroup => (
  ...
  return (
    ...
    >
      ...
      <Grid
        ...
        mt={{ base: 8, lg: 0 }}
      >
        { Gemefi_LINKS.map(link => <FooterLinkItem { ...link } key={ link.text }/>) }
      </Grid>
    </Grid>
  );
};
